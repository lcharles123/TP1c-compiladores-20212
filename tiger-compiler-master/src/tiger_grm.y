...
expsemicolonlist:
    %empty       {$$ = NULL;}   
|   exp       {         $$ = absyn_new_exp_list ($1, NULL);       }
|   exp SEMICOLON expsemicolonlist{         $$ = absyn_new_exp_list ($1, $3);       }
|   error SEMICOLON expsemicolonlist{         $$ = absyn_new_exp_list (NULL, $3);       }   ;

funcallargslist:
    %empty       {$$ = NULL;}   
|   exp       {         $$ = absyn_new_exp_list ($1, NULL);       }   
|   exp COMMA funcallargslist       {         $$ = absyn_new_exp_list ($1, $3);       }   
|   error COMMA funcallargslist       {         $$ = absyn_new_exp_list (NULL, $3);       }   ;


exp:       lvalue       {$$ = absyn_new_var_exp (errm_tok_pos, $1);}   
|   NIL       {$$ = absyn_new_nil_exp (errm_tok_pos);}   
|   INT       {$$ = absyn_new_int_exp (errm_tok_pos, $1);}   
|   STRING       {$$ = absyn_new_str_exp (errm_tok_pos, $1);}   
|   sequence       {$$ = $1;}   
|   negation       {$$ = $1;}   
|   funcall       {$$ = $1;}   
|   infix       {$$ = $1;}   
...
|   while       {$$ = $1;}   
|   for       {$$ = $1;}   
|   BREAK       {$$ = absyn_new_break_exp (errm_tok_pos);}   
|   let       {$$ = $1;}   ;

   sequence:       LPAREN expsemicolonlist RPAREN       {$$ = absyn_new_seq_exp (errm_tok_pos, $2);}
   |   LPAREN error RPAREN       {$$ = absyn_new_seq_exp (errm_tok_pos, NULL);}   ;

   negation:
    MINUS exp %prec UMINUS       {         $$ = absyn_new_op_exp (errm_tok_pos,                                ABSYN_MINUS_OP,                                absyn_new_int_exp (errm_tok_pos, 0),                                $2);       };
   
funcall:       typeid LPAREN funcallargslist RPAREN       {$$ = absyn_new_call_exp (errm_tok_pos, $1, $3);}   
|   typeid LPAREN error RPAREN       {$$ = absyn_new_call_exp (errm_tok_pos, $1, NULL);}
;

   infix:
    exp PLUS exp       {$$ = absyn_new_op_exp (errm_tok_pos, ABSYN_PLUS_OP, $1, $3);}   
|   exp MINUS exp       {$$ = absyn_new_op_exp (errm_tok_pos, ABSYN_MINUS_OP, $1, $3);}   
|   exp TIMES exp       {$$ = absyn_new_op_exp (errm_tok_pos, ABSYN_TIMES_OP, $1, $3);}   
|   exp DIVIDE exp       {$$ = absyn_new_op_exp (errm_tok_pos, ABSYN_DIVIDE_OP, $1, $3);}   
|   exp AND exp       {         $$ = absyn_new_if_exp (errm_tok_pos,
                             $1,                                $3,
                             absyn_new_int_exp (errm_tok_pos, 0));       }
   |   exp OR exp
    {         $$ = absyn_new_if_exp (errm_tok_pos,                                $1,
