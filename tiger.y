...
/*%token ABREPAR FECHAPAR ABRECHAV FECHACHAV ABRECOL FECHACOL MENOS MAIS VEZES DIVIDIR IGUAL DIFERENTE MENOR MENORIG MAIOR MAIORIG E OU OF ATRIBUI IF THEN ELSE WHILE DO FOR TO BREAK LET IN END TIPO DOISPONTOS VAR FUNCTION PONTO PONTOVIRG VIRG ID <intVal> NUM STRINGT ARRAY*/
...
|     LET decs IN expseq END	        {printf("exp \t-> LETdecsINexpseqEND\n");$$=novo_let(tokpos, let_t, $2, $4, tab);};
// declist e expseqOK    
...
vardec : 
     ...
     $$=novo_vardec(tokpos, normalvar_t, $2, "0", $4, tab);}  //var sem tipo declarado
     
|    VAR ID DOISPONTOS typeid ATRIBUI exp { printf("vardec \t-> VAR ID : typeid := exp\n");
    $$=novo_vardec(tokpos, normalvar_t, $2, $4, $6, tab);}; //var com tipo
...
typeid : 
    ID                                  { printf("typeid \t-> ID()\n"); T_instalar(tab, $1, ty_nome, NULL); }; //retornar o endereço em formato char* , evitar mexer na gramática
